...
- Rotas (o front chama `/api/<rota>?path=<rota>`): `lojas`, `parceiros`, `veiculos`, `leads`, `importar`, `refresh`, `auth`. Protegidas por `PAINEL_TOKEN`: `resumo`, `testeemail`, `conteudo`. A rota `avaliacoes` devolve as avaliações reais do Google para a faixa da home (guardadas em `config`, renovando uma loja por dia).
...
11. **Avaliações do Google:** a faixa da home mostra avaliações reais (4 e 5 estrelas, com texto) pela API oficial (Places API New), e aparece só quando existe `GOOGLE_PLACES_KEY` na Vercel. Na primeira vez busca todas as lojas; depois, uma consulta por dia, sempre na loja mais antiga (a pedido do Luiz). Com mais de 30 lojas, passar para duas por dia, porque o Google só deixa guardar por 30 dias. A mesma consulta atualiza a nota da loja. A leitura antiga (`notaGoogle()`, pela busca do Google) não funciona mais, porque o Google exige JavaScript. Teste: `node testes/avaliacoes.mjs`.
